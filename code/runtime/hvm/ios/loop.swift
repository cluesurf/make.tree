...
func runIo(input: (ctx: IoContext, term: UInt64)) -> HvmValue {
    ...
    while true {
        ...
        if name == ids.ioDone {
            ...
            let value = api.wnf(api.heapRead(loc))
            return fromTerm(input: (ctx: ctx.marshal, term: value))
        }

        if name == ids.ioCall {
            let loc = api.termVal(current)
            let primNameTerm = api.wnf(api.heapRead(loc))
            ...
            let arg = api.wnf(api.heapRead(loc + 1))
        ...
        if name == ids.ioBind {
            let loc = api.termVal(current)
            let action = api.heapRead(loc)
            let cont = api.heapRead(loc + 1)
