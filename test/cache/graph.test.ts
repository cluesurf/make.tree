...
describe('DepGraph', () => {
  it('tracks forward and reverse dependencies', () => {
    ...
    expect(g.deps.get('a.tree')?.has('b.tree')).toBe(true)
    expect(g.rdeps.get('b.tree')?.has('a.tree')).toBe(true)
  })
  ...
  it('serializes and deserializes round-trip', () => {
    ...
    expect(g2.deps.get('x.tree')?.has('y.tree')).toBe(true)
    expect(g2.deps.get('y.tree')?.has('z.tree')).toBe(true)
    expect(g2.rdeps.get('y.tree')?.has('x.tree')).toBe(true)
    expect(g2.rdeps.get('z.tree')?.has('y.tree')).toBe(true)
  })
