...
function desugarTask(task) {
    const card = { file: 'test.tree', list: [task] };
    const { book } = desugarCard({ card });
    return book.get(task.name);
}
/** Helper: desugar flow with empty scope. */
function flowToTerm(flow) {
    return desugarFlow({
        flow,
...
describe('term/desugar', () => {
        ...
        it('desugars task with save in flow', () => {
            ...
            if (term.form === 'ann' && term.val.form === 'lam') {
                ...
                if (body.form === 'let') {
                    ...
                    if (letBody.form === 'var')
                        expect(letBody.name).toBe('result');
                }
            }
        });
    });
    describe('form', () => {
        it('desugars simple enum form to ADT', () => {
            ...
            const card = { file: 'test.tree', list: [form] };
            const { book } = desugarCard({ card });
        ...
        it('desugars form nat with recursive constructor', () => {
            ...
            if (term.form === 'adt') {
                ...
                if (succTele.form === 'ext') {
                    ...
                    if (succTele.typ.form === 'ref')
                        expect(succTele.typ.name).toBe('nat');
                }
            }
        });
    });
    ...
    describe('end-to-end: desugar + reduce', () => {
        it('desugared identity function reduces correctly', () => {
            ...
            const app = { form: 'app', func: val, argm: { form: 'num', val: 42 } };
