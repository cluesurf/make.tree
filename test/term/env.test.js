...
import { envPure, envBind, envFail, envRun, envInit, envLog, envGetBook, envGetFill, envGetLogs, envFill, envSusp, envTakeSusp, envSnapshot, envRewind, envTry, envMap, envAll, envFreshMeta, envResetMeta, } from '@/term/env';
function emptyState() {
...
describe('term/env', () => {
    ...
    it('log adds info entries', () => {
        const state = emptyState();
        const info = { form: 'vague', name: 'test' };
        const env = envBind({
            env: envLog(info),
            fn: () => envGetLogs(),
        });
    ...
    it('getBook returns the book', () => {
        const book = new Map([['add', { form: 'ref', name: 'add' }]]);
    ...
    it('try recovers from failure', () => {
        const state = emptyState();
        const env = envTry({ env: envFail(), fall: 99 });
        const result = envRun({ env, state });
    ...
    it('all fails if any computation fails', () => {
        const state = emptyState();
        const env = envAll([envPure(1), envFail(), envPure(3)]);
        const result = envRun({ env, state });
        expect(result).toBeNull();
    });
