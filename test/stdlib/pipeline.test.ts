...
describe('kink form', () => {
  ...
  it('has no case arms (struct-like)', () => {
    const card = loadStdlib('@cluesurf/base/code/base/form/kink')
    const book = desugar(card)
    const term = book.get('kink')!

    if (term.form !== 'adt') throw new Error('expected adt')
    // kink has link fields but no case arms
    // It should have 0 constructors (no case keyword used)
    expect(term.ctrs.length).toBe(0)
  })
...
describe('codegen for stdlib ADTs', () => {
  it('ADT-only book is skipped in TypeScript (type-level)', () => {
    const card = loadStdlib('@cluesurf/base/code/base/form/boolean')
    const book = desugar(card)
    const ts = castTS({ book })

    // ADTs are type-level, no runtime code emitted for standalone ADTs
    expect(ts).toBe('')
  })
